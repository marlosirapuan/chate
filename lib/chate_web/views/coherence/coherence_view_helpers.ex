defmodule ChateWeb.Coherence.ViewHelpers do
  @moduledoc """
  Helper functions for Coherence Views.
  """
  use Phoenix.HTML
  alias Coherence.Config
  import ChateWeb.Gettext

  @type conn :: Plug.Conn.t()
  @type schema :: Ecto.Schema.t()

  @seperator {:safe, "&nbsp; | &nbsp;"}
  @helpers ChateWeb.Router.Helpers
  ...
  * :register - customize the register link text (#{@register_link})
  ...
  def coherence_links(conn, :layout, opts) do
    ...
    signout_class = Keyword.get(opts, :signout_class, "navbar-form nav-link")
    ...
    if Coherence.logged_in?(conn) do
      ...
      signin_link =
        content_tag(
          list_tag,
          link(signin, to: coherence_path(@helpers, :session_path, conn, :new), class: "nav-link")
        )
      ...
      if Config.has_option(:registerable) && register do
        [
          content_tag(
            list_tag,
            link(register, to: coherence_path(@helpers, :registration_path, conn, :new), class: "nav-link")
          ),
          signin_link
        ]
      else
